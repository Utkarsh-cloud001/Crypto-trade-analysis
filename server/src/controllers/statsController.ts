...
export const getStats = async (req: Request, res: Response) => {
    try {
        ...
        const trades = await Trade.find(dateFilter);
        ...
        const dayStats: any = {};
        closedTrades.forEach(trade => {
            const day = new Date((trade as any).createdAt).getDay();
            if (!dayStats[day]) {
        ...
        const hourStats: any = {};
        closedTrades.forEach(trade => {
            const hour = new Date((trade as any).createdAt).getHours();
            if (!hourStats[hour]) {
